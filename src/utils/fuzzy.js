...
const fuzzy = {
  eolicHealth: async (measurements) => {
    try {
      ...
      const health = await axios.post(`${PROCESSING_URL}/fuzzy/eolic`, {
        wind,
        potency,
      });
  ...
  solarHealth: async (measurements) => {
    try {
      ...
      const health = await axios.post(`${PROCESSING_URL}/fuzzy/solar`, {
        temperature,
        potency,
      });
  ...
  notify: async (data) => {
    try {
      ...
      if (
        health.data.health_status == "Não Saudável" &&
